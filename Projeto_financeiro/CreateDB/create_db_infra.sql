	CREATE DATABASE infra_financeiro;
	GO
    ...
    SELECT
        ...
        t2.name,
        c2.name
    FROM projeto_financeiro.sys.foreign_keys AS fk
    INNER JOIN projeto_financeiro.sys.tables AS t ON t.object_id = fk.referenced_object_id 
    INNER JOIN projeto_financeiro.sys.tables AS t2 ON t2.object_id = fk.parent_object_id
    INNER JOIN projeto_financeiro.sys.foreign_key_columns AS fkc ON fkc.constraint_object_id = fk.object_id
    INNER JOIN projeto_financeiro.sys.columns AS c ON c.column_id = fkc.referenced_column_id AND c.object_id = t.object_id
    INNER JOIN projeto_financeiro.sys.columns AS c2 ON c2.column_id = fkc.parent_column_id AND c2.object_id = t2.object_id
    ...
    SELECT name
    FROM projeto_financeiro.sys.tables
